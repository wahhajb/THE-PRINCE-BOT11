...
let handler = async (m, { args, usedPrefix, command }) => {
    ...
    if (args[0]) {
        if (args[0] < 1 || args[0] > 99) throw `الحد الادنى 1 و الاقصى 99!`
        let { index, latin, arabic, translation_id, translation_en } = json.find(v => v.index == args[0].replace(/[^0-9]/g, ''))
        return m.reply(` اسـماء الله الـحسنى ${index}
${arabic}
${translation_id}
`.trim())
    }
...
handler.help = ['الأسماء الحسنى[1-99]']
handler.tags = ['قران']
handler.command = /^الأسماء-الحسنى$/i
